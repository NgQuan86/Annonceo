...
$afficheFormCategories = $pdo->query(" SELECT DISTINCT categorie FROM annonce ORDER BY categorie ASC ");
...
$afficheFormMembres = $pdo->query(" SELECT DISTINCT membre FROM annonce ORDER BY membre ASC ");
...
// 01-tout l'affichage par categorie
    if(isset($_GET['categorie'])){
        ...
        $afficheAnnonce = $pdo->query(" SELECT * FROM annonce WHERE categorie = '$_GET[categorie]' ORDER BY prix ASC ");

        // pour les onglets categories
        $pageTitle = "Annonce de " . $_GET['categorie'];
    }
...
// 03-tout l'affichage par Région
