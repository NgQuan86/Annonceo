...
$recupAnnonce = $pdo->prepare("SELECT titre, description_longue, prix, membre.nom , note.note AS group_annonce FROM annonce, categorie, membre WHERE id_membre = membre_id AND id_note = note_id AND id_annonce = :id_annonce");
$recupAnnonce->bindParam(':id_annonce', $_GET['id_annonce']);
$recupAnnonce->execute();

// POUR RECUPERER LES PHOTOS
if($recupAnnonce->rowCount() > 0) {
    $infosAnnonce = $recupAnnonce->fetch(PDO::FETCH_ASSOC);

    $listePhotos_annexes = $pdo->prepare("SELECT * FROM photo WHERE id_photo = :id_photo");
    $listePhotos_annexes->bindParam(':id_photo', $infos_annonce['photo_id']);
    $listePhotos_annexes->execute();

    $infosPhotos_annexes = $listePhotos_annexes->fetch(PDO::FETCH_ASSOC);
} else {
...
<div class="container-fluid">
    ...
    <div class="row">
        ...
        <div>
            <h4><?= $infoAnnonce['titre'] ?></h4>
            <p>fdjkfhdsfskfskhfqkhfsfjk</p>
        </div>
    </div>
</div>
